...
export class DsbmobileClient {
    ...
    for (const item of items) {
      if (item.ConType === 2 && item.Childs && item.Childs.length > 0) {
        ...
        for (const [i, validChild] of validChildren.entries()) {
          ...
          const title = multiPage ? `${item.Title} (Seite ${i + 1})` : item.Title;
    ...
    try {
      ...
      for (const timetable of timetables) {
        try {
          ...
          // The HTML is encoded in iso-8859-1 (latin-1) — decode it properly
          const htmlText = new TextDecoder('iso-8859-1').decode(response.data);
          const plan = this.parseSubstitutionHtml(
            htmlText,
            timetable.title,
            timetable.date,
            timetable.url,
  ...
  private parseSubstitutionHtml(
    ...
    for (const rowMatch of allRows) {
      ...
      // Class header row: single cell with class name (e.g. "05b  05b" or just "05b")
      if (cells.length === 1) {
